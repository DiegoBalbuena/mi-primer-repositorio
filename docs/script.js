const apiKey = 'af7ec40a5997c1318ecd4dbd179cea5e'; // Reemplaza con tu clave API
...
async function fetchPopularMovies() {
    try {
        ...
        displayMovies(data.results);  // Pasar las películas obtenidas a la función de visualización
    } catch (error) {
...
async function showMovieDetails(movieId) {
    try {
        ...
        `;
        movieDetails.classList.remove('hidden');  // Mostrar el contenedor de detalles
    } catch (error) {
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            const response = await fetch(`${apiUrl}/search/movie?api_key=${apiKey}&query=${encodeURIComponent(query)}&language=es-ES&page=1`);
            const data = await response.json();
            displayMovies(data.results);  // Pasar los resultados de la búsqueda a la función de visualización
        } catch (error) {
